...
public class CreditCardController {
    ...
    private void addBalance(UpdateBalancePayload payload) {
        ...
        for (int index = 0; index < balanceHistories.size(); index++) {
            ...
            if (balanceHistory.getDate().isEqual(payloadDate) ||
                    (payloadDate.isAfter(balanceHistory.getDate()) &&
...
//                break;
    ...
    public ResponseEntity<Double> getCreditCardBalanceForDate(@RequestParam String number, @RequestParam LocalDate date) {
        ...
        for (int index = 0; index < balanceHistories.size(); index++) {
            ...
            if (balanceHistory.getDate().equals(date)) {
                return ResponseEntity.ok(balanceHistory.getBalance());
            } else if (balanceHistory.getDate().isAfter(date)) {
                return ResponseEntity.ok(creditCard.getBalanceHistories().get(index-1).getBalance());
            }
        }
